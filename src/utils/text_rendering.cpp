...
const GLchar *const textfragmentshader_source =
    ...
    "uniform vec3 textColor;\n" // Add a uniform for the text color
    "in vec2 texCoords;\n"
    "out vec4 fragColor;\n"
    "void main()\n"
    "{\n"
    "    float alpha = texture(tex, texCoords).r;\n"
    "    fragColor = vec4(textColor, alpha);\n" // Use the text color and alpha from the texture
    "}\n"
    "\0";
...
void TextRendering_Init() {
	...
	// Get the uniform location for textColor
	GLuint textColor_uniform = glGetUniformLocation(textprogram_id, "textColor");
	if (textColor_uniform == -1) {
		fprintf(stderr, "ERROR: Uniform 'textColor' not found in shader program.\n");
	}
	glCheckError();
...
void TextRendering_PrintStringC(GLFWwindow* window, const std::string &str, float x, float y, glm::vec3 color, float scale = 1.0f){
	scale *= textscale;
	int width, height;
	...
	GLuint textColor_uniform = glGetUniformLocation(textprogram_id, "textColor");
	if (textColor_uniform != -1) {
		glUniform3f(textColor_uniform, color.r, color.g, color.b); // Set text color
	}
...
void TextRendering_PrintString(GLFWwindow* window, const std::string &str, float x, float y, float scale = 1.0f){
    TextRendering_PrintStringC(window, str, x, y, {0.0f, 0.0f, 0.0f}, scale);
}
...
void TextRendering_ShowFramesPerSecond(GLFWwindow *window) {
	...
	TextRendering_PrintStringC(window, buffer, 1.0f - (numchars + 1) * charwidth, 1.0f - lineheight, {0.78f, 0.89f, 0.22f}, 1.0f);
}
